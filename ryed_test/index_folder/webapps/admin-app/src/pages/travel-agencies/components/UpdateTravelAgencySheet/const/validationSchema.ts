...
export const validationSchema = Yup.object().shape({
  ...
  exclusiveDispatcherId: Yup.string(),
  exclusivePeriod: Yup.string(),
  commissionAmount: Yup.number().min(0, 'Must be >= 0').required('Commission amount is required'),
  commissionType: Yup.string().oneOf(['PERCENTAGE', 'FIXED']).required('Commission type is required'),
  commissionIncludedInPrice: Yup.boolean().required('Commission included in price is required'),
