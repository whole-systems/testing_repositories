...
import { useUpdateDispatcherSheet } from './hooks/useUpdateDispatcherSheet';
...
import { TDispatcherRow } from '../DispatchersTable/consts/columns';
import { VatPicker } from '@/components/VatPicker/VatPicker.tsx';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/Select/Select';

interface Props {
  dispatcher: TDispatcherRow;
}

export const UpdateDispatcherSheet: FC<Props> = ({ dispatcher }) => {
  const { data, handlers } = useUpdateDispatcherSheet(dispatcher.id);

  return (
    <div>
      <Sheet
        open={data.isOpenSheet}
        onOpenChange={() => handlers.setIsOpenSheet(false)}
      >
        <Button
          variant="ghost"
          size="icon"
        ...
        >
          <SheetHeader>
            <SheetTitle>Update {dispatcher.companyName}</SheetTitle>
          </SheetHeader>
              ...
              <div className="flex-1 overflow-auto flex w-full">
                <div className="flex flex-col h-full overflow-auto w-full">
                  <div className="w-full flex flex-col gap-4 flex-wrap">
                        ...
                        <div className=" flex flex-col h-full  justify-center items-center">
                          ...
                          >
                            <div className="flex w-full h-full flex-col justify-center items-center">
                              {data.dispatcher?.logoUrl || data.logoPreview ? (
                                <div className="w-full h-full rounded p-2">
                                  <img
                                    src={
                                      data.dispatcher?.logoUrl ||
                                      (data.logoPreview as string)
                                    }
                                    alt={'logo'}
                    ...
                    <Card>
                      ...
                      <CardContent className="flex flex-col gap-4">
                          ...
                          <div className="">
                            <div>
                              ...
                              <Input
                                ...
                                name="companyName"
                                value={data.formik.values.companyName}
                                onChange={data.formik.handleChange}
                              />

                              <div>
                                <span className="text-sm text-red-600">
                                  {data.formik.errors.companyName}
                                </span>
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="my-4  w-full h-full ">
                          <div className="font-semibold text-lg mb-4 flex ">
                            Company location:
                          </div>
                          <GooglePlaceAutocomplete
                            onChooseLocation={(location) => {
                              handlers.handleLocation(location);
                            }}
                            value={data.formik.values.address ?? ''}
                            name="location"
                            placeholder="Location"
                          />
                          <div>
                            <span className="text-sm text-red-600">
                              {data.formik.errors.lat}
                            </span>
                          </div>
                        </div>
                        <div className="w-full">
                    ...
                    <Card>
                      <CardContent>
                        <div className="my-4 w-full flex justify-between lg:w-1/2">
                          <div className="text-lg font-semibold mb-2">
                            Multi Driver Approval:
                          </div>
                          <Switch
                            name={'isMultiDriverApproval'}
                            checked={
                              data.formik.values
                                .isMultiDriverApproval as boolean
                            }
